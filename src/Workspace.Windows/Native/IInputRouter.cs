// Ported from apps/host-windows/src/Workspace.Host/Windows/IInputRouter.cs at vNext M1 3474aa0.
...
public static class InputCoordinateMapper
{
    public static ScreenPoint Map(double x, double y, WindowBounds bounds)
    {
        ...
        return new ScreenPoint(
            checked(bounds.Left + (int)Math.Round(x * (bounds.Width - 1), MidpointRounding.AwayFromZero)),
            checked(bounds.Top + (int)Math.Round(y * (bounds.Height - 1), MidpointRounding.AwayFromZero)));
    }
}
